...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [
			"5511934713306@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"5511934713306@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
        ...
        const limitAdd = (sender) => {
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    reply('*[LINK DE GRUPO DETECTADO!]*\nVOU TE REMOVER DO GRUPO DA PRÓXIMA VEZ PRESTE ATENÇÃO NAS REGRAS.').then(() => {
                        client.groupRemove()
                    })
                }
            }
         ...
         case 'lirik':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('5511934713306@s.whatsapp.net', options, text, {quoted: mek})
				...
				default:
                  ...
                  reply(`Desculpe man *${pushname2}*, Comando *${prefix}${command}* comando nn registrado no menu! pfv chame o bot novamente *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[FADHILBOT]','Yellow'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
