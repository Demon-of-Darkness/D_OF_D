	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Eai @${num.split('@')[0]}\nLei as regras pfvr *${mdata.subject}*`
				...
				teks = `Adeus @${num.split('@')[0]}foi um prazer ter vc conosco👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Tik Tok n é bom mas...kd a url?')
